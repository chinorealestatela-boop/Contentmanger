...
export async function createTask(_prev: TaskActionState, formData: FormData): Promise<TaskActionState> {
  ...
  const assigneeId = parsed.data.assigneeId || scope.userId;
  const assignee = await prisma.user.findUnique({ where: { id: assigneeId }, select: { isActive: true } });
  if (!assignee || !assignee.isActive) {
    return { error: "The selected assignee is no longer available. Please refresh the page and try again." };
  }
  ...
  if (parsed.data.customerId) {
    try {
      ...
      console.error("createTask: activity log failed (task was still saved)", err);
    }
  }
...
export async function updateTask(_prev: TaskActionState, formData: FormData): Promise<TaskActionState> {
  await requireScope();
  ...
  try {
    ...
    revalidatePath("/calendar");
    if (task.customerId) revalidatePath(`/customers/${task.customerId}`);
    return { success: "Task updated.", taskId: task.id };
  } catch (err) {
    console.error("updateTask failed", err);
    return { error: "Couldn't save changes to this task — please try again." };
  }
}
