 ...
 * stored — see computeRemainingBalance). */
export async function markPaymentPaid(_prev: SimpleActionState, formData: FormData): Promise<SimpleActionState> {
  ...
  if (!parsed.success) return { error: parsed.error.issues[0]?.message ?? "Invalid input." };
  const { paymentId, amountPaid, notes } = parsed.data;
  ...
  await prisma.payment.update({
    where: { id: paymentId },
    data: { status: newStatus, amountPaid, paidAt: new Date(), paidById: scope.userId, notes: notes ?? payment.notes },
  });
  ...
  await logActivity({
    ...
    description: `Payment ${newStatus === "PAID" ? "marked paid" : "marked partially paid"} — $${amountPaid.toLocaleString()} of $${payment.amount.toLocaleString()} due ${new Date(payment.dueDate).toLocaleDateString()}.`,
    actorId: scope.userId,
    metadata: { paymentId },
  });
...
export async function updatePayment(_prev: SimpleActionState, formData: FormData): Promise<SimpleActionState> {
  ...
  // date changing is that the old schedule no longer applies.
  await prisma.payment.update({
    where: { id: paymentId },
    data: { amount, dueDate: new Date(`${dueDate}T00:00:00`), notes, remindersSent: "[]" },
  });
