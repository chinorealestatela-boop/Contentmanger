...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 p-4 sm:p-6 lg:p-8">
      <div>
        <h1 className="font-display text-3xl font-medium text-[var(--text)]">Tasks</h1>
        <p className="text-[13px] text-[var(--text-muted)]">Every call, deposit, driver assignment, and reminder you own — automations create these too.</p>
      </div>
        ...
        {VIEWS.map((v) => (
          <Link
            ...
            className={cn(
              ...
              view === v.key ? "bg-[var(--brand)] text-[#14120a]" : "text-[var(--text-muted)] hover:bg-white/[0.05]"
            )}
          >
